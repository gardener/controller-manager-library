...
type Patch interface {
	...
	Data(obj ObjectData) ([]byte, error)
}
...
func (s *patch) Data(obj ObjectData) ([]byte, error) {
	return s.data, nil
}
...
// Data implements Patch.
func (s *mergeFromPatch) Data(obj ObjectData) ([]byte, error) {
	original := s.from
	modified := obj
	...
	if s.opts.OptimisticLock {
		...
		original = original.DeepCopyObject().(ObjectData)
		original.SetResourceVersion("")

		modified = modified.DeepCopyObject().(ObjectData)
...
func MergeFrom(obj ObjectData) Patch {
	return &mergeFromPatch{patchType: types.MergePatchType, createPatch: createMergePatch, from: obj}
}

// MergeFromObject creates a Patch that patches using the merge-patch strategy with the given object as base.
func MergeFromObject(obj Object) Patch {
	return MergeFrom(obj.Data())
}

// MergeFromWithOptions creates a Patch that patches using the merge-patch strategy with the given object as base.
// See MergeFrom for more details.
func MergeFromWithOptions(obj ObjectData, opts ...MergeFromOption) Patch {
...
// MergeFromObjectWithOptions creates a Patch that patches using the merge-patch strategy with the given object as base.
// See MergeFrom for more details.
func MergeFromObjectWithOptions(obj Object, opts ...MergeFromOption) Patch {
	return MergeFromWithOptions(obj.Data(), opts...)
}
...
func StrategicMergeFrom(obj ObjectData, opts ...MergeFromOption) Patch {
	options := &MergeFromOptions{}
	for _, opt := range opts {
		opt.ApplyToMergeFrom(options)
	}
...
// StrategicMergeFrom creates a Patch that patches using the strategic-merge-patch strategy with the given object as base.
func StrategicMergeFromObjcet(obj Object, opts ...MergeFromOption) Patch {
	return StrategicMergeFrom(obj.Data(), opts...)
}
...
// Data implements Patch.
func (p mergePatch) Data(obj ObjectData) ([]byte, error) {
...
// Data implements Patch.
func (p applyPatch) Data(obj ObjectData) ([]byte, error) {
	// NB(directxman12): we might technically want to be using an actual encoder
	// here (in case some more performant encoder is introduced) but this is
	// correct and sufficient for our uses (it's what the JSON serializer in
	// client-go does, more-or-less).
