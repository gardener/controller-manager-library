...
func ForServer(name string) Configuration {
	return Configuration{*mappings.NewDefinition(TYPE_SERVER, name)}
}

func ForServerGroup(name string) Configuration {
	return Configuration{*mappings.NewDefinition(TYPE_GROUP, name)}
}

func (this empty) ForServer(name string) Configuration {
	return ForServer(name)
}

func (this empty) ForServerGroup(name string) Configuration {
	return ForServerGroup(name)
}
